...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{images/5-gnn-algorithm/ground-truth.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/5-gnn-algorithm/heatmap-network.png}
...
\begin{equation}
{V}_{ij} = H {V}_{ij} H^{T} + \sigma_{0}^{2}
...
Figure \ref{fig:example-application-1} shows an example of the GNN algorithm applied to the simulated network in Figure \ref{fig:heat-map}. Figure \ref{fig:mc-example-1} displays the graph network where an initial CCA has been applied. Three smaller subgraphs have been identified as shown by the separate colours. Nodes with $\sigma_e^2 > 0.8$ are not shown here, as clustering was not possible. The subsequent extracted track candidates from this network are shown in Figure \ref{fig:mc-example-2}, where seven distinct tracks are observed. Outlier edge connections have been successfully identified. This particular simulation shows 100\% efficiency of track reconstruction with respect to MC truth, as all seven truth tracks were extracted within two stages. This indicates that the GNN algorithm behaved as expected, first resolving ambiguities in ``cold'' regions.
