...
\begin{figure}[htbp]%
    ...
    \caption{Extracted track candidates post Stage 1 of the GNN algorithm applied to volume 7 of a simulated TrackML detector event, shown from the view of the a) $x$-$y$ plane and b) $r$-$z$ plane. }%
...
The proportion of nodes removed from the main graph network during Stage 1 and Stage 2 were 68\% and 11\% respectively.

% the metrics on the outliers succesffully  detected at each stage, precision/recall
% precision 85% and recall was 90%
% confusion matrix
...
\begin{figure}[htbp]%
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{TODO...Stage 1 confusion matrix for the endcap only}
...
\begin{figure}[htbp]%
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{TODO...Stage 2 confusion matrix for the endcap only}
...
%particle purity, track purity, need to present the average over many events here
