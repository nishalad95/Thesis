...
The KF track fit is applied simultaneously in both the $x$-$y$ and $r$-$z$ planes, in order to assess the quality of the fit using two different models.
...
Once the KF track fits are complete in both planes, the corresponding chi2 distance is calculated for each track segment (node-pair), the resulting chi2 statistic is calculated and the degrees of freedom (no. of measurements - no. of track params) is calculated. The p-value representing the track fit for the entire track is then calculated using the degrees of freedom and chi2 statistic. if p larger than 0.01 then track is extracted.