...
Extrapolation and track extraction are handled independently for the $x$-$y$ and $r$-$z$ planes, with each stage a separate KF. The filter parameters are summarized in Table \ref{tab:kf-instance-variables} and the derivations are described in Sections \ref{chapter-6-start-of-derivation} - \ref{chapter-6-end-of-derivation}.



\begin{table}[!htbp]
\caption{Summary of parameters used in implementation of KFs for state extrapolation and track extraction, in the $x$-$y$ and $r$-$z$ planes.}
\begin{center}
\begin{tabular}{lccc}
\toprule
Instance Variable Description & \multicolumn{3}{c}{Parameters for $x$-$y$ Plane} \\
& Symbol  & Extrapolation  & Track Extraction  \\
\hline
\rule{0pt}{3ex}%
...
\toprule
Instance Variable Description & \multicolumn{3}{c}{Parameters for $r$-$z$ Plane} \\
& Symbol  & Extrapolation  & Track Extraction  \\
\hline
\rule{0pt}{3ex}%
...
where $\sigma_{xy} = 300\mu$m. The corresponding Mahalanobis distance is given by Eq \eqref{eqn:mahalanobis-distance-trackml-xy}.
...
If $\Delta \chi^{2} < 1$, $\tilde{X}$ is fully extrapolated to its neighbour node using the KF update.
