...
For a set of $n$ nodes representing a track candidate, the sequential $y$-measurements are $\{y_0, y_1, ..., y_{(n-1)} \}$, where $y_0$ is the $y$-measurement of the node with the smallest radius and $y_{(n-1)}$ is the $y$-measurement of the node with the largest radius. The KF is initialised with the following three-dimensional filter state estimate, $x_{xy}$, given by Eq \eqref{eqn:kf-track-fit-initialised-state-xy}.


\begin{equation}
\quad x_{xy} = \begin{bmatrix} y_{(n-1)} \\ 0 \\ 0 \end{bmatrix} 
\label{eqn:kf-track-fit-initialised-state-xy}
\end{equation}

and the covariance matrix, $P_{rz}$, given by Eq \eqref{eqn:kf-track-fit-initialised-cov-xy}

\begin{equation}
\quad P_{xy} = \begin{bmatrix} \sigma_{xy}^2 & 0 & 0 
                                    \\ 0 & 1 & 0 
                                    \\ 0 & 0 & 1 \end{bmatrix} 
...
The state transition Jacobian $F_{xy}$ is derived using the OU model \cite{OU} and is given by \eqref{eqn:state-transition-jacobian-ou},
